...
def data_set(num):
    dir = r'Z:\ALL\CODE ALL\PYTHON ALL\python\ACO-AS-main\data'
    data = []
    for n in os.listdir(dir):
        subdir_path = os.path.join(dir, n)
...
def heuristic_info(weight, capacity):
...
def main():
    ...
    for iter in range(150):
        ...
        for ant_num in range(ant_count):
            ...
            while capacity >= 0:
                chosen_obj = [
                    ((pheromone_track[item]) ** alfa) * ((heuristic_info(weight, profit)) ** beta) if availability[item]
                                                                                                      == True else 0 for item in range(len(weight))]
